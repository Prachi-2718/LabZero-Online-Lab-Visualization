  ...
  return (
    <div className={`relative min-h-screen bg-[var(--bg-deep)] text-[var(--text-primary)] font-sans selection:bg-[#7DD3FC]/30 overflow-hidden transition-colors duration-500 ${theme === 'light' ? 'light-mode' : ''}`}>
      ...
      <main className="pt-32 pb-24 px-6 md:px-12 max-w-[1400px] mx-auto space-y-32">
        ...
        {/* Theory to Visual */}
        <section className="bg-[var(--bg-panel)] rounded-[40px] p-8 md:p-12 border border-[var(--border-glass)] shadow-sm flex flex-col lg:flex-row items-center gap-12">
            <div className="lg:w-1/3">
               <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest">Abstract to Visual</span>
               <h2 className="text-4xl md:text-5xl font-display font-bold mt-4 mb-6 text-[var(--text-primary)]">From Theory to <span className="text-[var(--color-secondary)]">Reality</span></h2>
               <p className="text-[var(--text-muted)] mb-8 leading-relaxed">We bridge the gap between abstract theory and real-world understanding through immersive 3D visualizations.</p>
               <button className="text-[var(--color-secondary)] font-medium flex items-center gap-2 hover:gap-3 transition-all">Learn More <ArrowRight size={16}/></button>
            </div>
            
            <div className="lg:w-2/3 flex flex-col md:flex-row items-center gap-6 w-full">
               <div className="flex-1 w-full bg-[var(--bg-deep)]/30 rounded-3xl p-8 border border-[var(--border-glass)] flex flex-col items-center justify-center text-center shadow-inner h-64">
                  <p className="text-sm font-medium text-[var(--text-muted)] mb-6">Abstract Theory</p>
                  <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/e/e5/Schrodinger_equation.svg/440px-Schrodinger_equation.svg.png" className="h-12 opacity-70 invert dark:invert-0" alt="Equation" />
                  <p className="text-xs text-[var(--text-muted)] mt-6">Schrödinger Equation</p>
               </div>
               
               <div className="w-12 h-12 rounded-full border border-[var(--border-glass)] bg-[var(--bg-panel)] flex items-center justify-center text-[var(--color-secondary)] shrink-0 shadow-sm z-10 -my-4 md:my-0 md:-mx-10">
                  <ArrowRight size={20} />
               </div>
               
               <div className="flex-1 w-full bg-gradient-to-br from-[var(--color-secondary)]/5 to-[var(--bg-deep)]/30 rounded-3xl p-8 border border-[var(--color-secondary)]/20 flex flex-col items-center justify-center h-64 relative overflow-hidden group">
                  <p className="text-sm font-medium text-[var(--color-secondary)] mb-6 absolute top-6 left-6">3D Visualization</p>
                  <div className="w-full h-full flex items-center justify-center shrink-0">
                     <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-[var(--color-secondary)] to-[var(--color-primary)] shadow-[0_0_40px_rgba(var(--color-secondary-rgb),0.4)] relative">
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[50%] border border-[var(--color-secondary)] rounded-[100%] rotate-45 opacity-50"></div>
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[50%] border border-[var(--color-primary)] rounded-[100%] -rotate-45 opacity-50"></div>
                     </div>
                  </div>
                  <button className="absolute top-6 right-6 w-8 h-8 bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center shadow-sm text-[var(--text-primary)] hover:bg-white/20 transition-colors">
                     <Maximize2 size={14} />
                  </button>
               </div>
            </div>
        ...
        {/* Stats Row */}
        <section className="bg-[var(--bg-panel)] rounded-[40px] md:rounded-full py-8 px-8 md:px-16 border border-[var(--border-glass)] shadow-sm flex flex-col md:flex-row justify-between items-start md:items-center gap-8 text-center max-w-5xl mx-auto mb-24">
